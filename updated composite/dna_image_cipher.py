...
# ===================================================================
# 3. Enhanced Encryption/Decryption with Rounds (RGB Support)
# ===================================================================
class EnhancedDNAImageCipher:
    ...
    def _process_image_channels(self, image: np.ndarray, process_func) -> np.ndarray:
        """Process each channel separately for RGB images"""
        if len(image.shape) == 3:  # RGB image
            channels = []
            for c in range(3):
                channel = process_func(image[:, :, c])
    ...
    def encrypt(self, image_path: str, output_path: str = "encrypted.png", rounds: int = 1):
        ...
        for i in range(rounds):
            ...
            encrypted_img = self._process_image_channels(encrypted_img, self._encrypt_round)
    ...
    def decrypt(self, image_path: str, output_path: str = "decrypted.png", rounds: int = 1, save: bool = True):
        enc_img = cv2.imread(image_path, cv2.IMREAD_COLOR)
        if enc_img is None:
        ...
        for i in range(rounds):
            ...
            decrypted_img = self._process_image_channels(decrypted_img, self._decrypt_round)
        
        # Convert back to BGR for saving
        if save:
            if len(decrypted_img.shape) == 3:
                decrypted_img = cv2.cvtColor(decrypted_img, cv2.COLOR_RGB2BGR)
...
# ===================================================================
# 4. Image Analysis Utilities (RGB Support)
# ===================================================================
...
def plot_histograms(orig, enc, save_path=None):
    if len(orig.shape) == 3 or len(enc.shape) == 3:
        # Handle color images
        ...
        for i in range(3):
            plt.subplot(2, 3, i+1)
            plt.hist(orig[:, :, i].ravel(), bins=256, range=(0, 255), color='red', alpha=0.7)
        ...
        for i in range(3):
            plt.subplot(2, 3, i+4)
            plt.hist(enc[:, :, i].ravel(), bins=256, range=(0, 255), color='blue', alpha=0.7)
        ...
        plt.subplot(121)
        plt.hist(orig.ravel(), bins=256, range=(0, 255), color='blue', alpha=0.7)
        ...
        plt.subplot(122)
        plt.hist(enc.ravel(), bins=256, range=(0, 255), color='red', alpha=0.7)
...
def correlation_coefficients(img):
    if len(img.shape) == 3:
        # For RGB images, calculate correlations for each channel
        horiz_corrs, vert_corrs, diag_corrs = [], [], []
        for c in range(3):
            h, v, d = correlation_coefficients_channel(img[:, :, c])
            horiz_corrs.append(h)
            vert_corrs.append(v)
            diag_corrs.append(d)
        return (
            np.mean(horiz_corrs),
        ...
        return correlation_coefficients_channel(img)
    
def correlation_coefficients_channel(img):
    """Calculate correlation coefficients for a single channel"""
    h, w = img.shape
    horiz = img[:, :-1].flatten()
...
def entropy(img):
    """Calculate entropy for both grayscale and RGB images"""
    if len(img.shape) == 3:
        # For RGB images, calculate entropy for each channel
        entropies = []
        for c in range(3):
            entropies.append(entropy_channel(img[:, :, c]))
        return np.mean(entropies)
    else:
        return entropy_channel(img)
    
def entropy_channel(img):
...
if __name__ == "__main__":
    # This is just to prevent execution when imported
    pass